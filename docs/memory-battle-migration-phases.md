...
1. `scripts/make-info-memory.mjs` を `make-info-highlow.mjs` 複製で作成。出力 `catalog/google_play_store_images/00005_memory-battle/info/{en,ja,zh}/`。
...
1. `scripts/make-video-memory.mjs` を `make-video-highlow.mjs` 複製で作成。540×960 録画→ffmpeg `scale=1080:1920` MP4。
...
- **Phase 5 完了**(commit e1564b5): INFO画像 `catalog/google_play_store_images/00005_memory-battle/info/{en,ja,zh}/` 各18画面=**54枚 全1080×1920(NG=0)**。scripts/make-info-memory.mjs。
- **Phase 6 完了**: MP4 `catalog/google_play_store_images/00005_memory-battle/video/{en,ja,zh}/` 各4本(vid1-main-practice/vid2-main-battle/vid3-intro/vid9-menu)=**12本 全1080×1920・実動画(frames166〜535・フレーム差分で動き確認)**。scripts/make-video-memory.mjs(LANGS対応)。*.mp4 は gitignore のため非コミット(ローカル成果物)。
