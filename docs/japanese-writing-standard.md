...
ゲーム内通貨・賭けの表現は app store 審査に関わる。次を厳守する（手本 = old-maid）。

- 通貨は **「ゲーム内COIN」**。**「仮想COIN」「仮想通貨」「暗号通貨」**など暗号資産/仮想通貨を匂わせる語は**禁止**（実通貨価値を連想させ審査落ちの原因）。
- 賭けは **「BET」「BET機能」**（ゲーム内の機能）として書く。`賭ける(gamble)` の語感は避ける。
  - 例：`仮想COINを賭けて` → `ゲーム内COINをBETして` ／ `選んだ賭け` → `選んだBET`。
  - 手本：old-maid「ゲーム内COINを使った、BET機能を追加」「ゲーム開始前にCOINをBETし」。
- 打ち消し文「現金・賞品・暗号資産などは使いません。ギャンブル目的のアプリではありません。」は**否定文＝保持してよい**。
- en/zh も `virtual coin/currency`・`加密/虚拟货币` 等で crypto を匂わせない。
