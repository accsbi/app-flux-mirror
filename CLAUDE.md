...
## 1c. フォールバック禁止（最重要・全実装にグローバル適用・再発厳禁）
- **フォールバックを書かない。** 値が無い＝`||`／`??`／三項の右辺に手書きキー・既定文字列・別ソース・「失敗時バンドル」「全滅時 config」「未注入時 config」を置かない。**無ければ `throw new Error(...)` で落として表面化**させる。
- **根本原因＝その場限りの独自実装。** フォールバックは症状。共有/共通(shared/common/config)を使わず作り直すから、繋がらない所を `||` で埋めてバグを隠す。**「フォールバックを足したくなった瞬間に手を止める＝独自実装のサイン」**。共有の正規ソース1本に直す（§1 と一体）。
- **機能は単一ソース**：版数=`__APP_VERSION__`（pubspec）/ 規約=外部ライブ`terms-of-use.json` のみ / 文言=config の唯一キー。複数段に分けない。取得失敗時は**出さない**（例: 規約ボタンを隠す）。
- **AAB/出荷物へ同梱したフォールバックは取り返しがつかない**（古い内容が永久に出る）。同梱データへ落とす設計自体を禁止。
- **バグか仕様か判別不能にしない**＝フォールバックで「動いて見える」状態を作らない（後で致命的になる）。詳細＝メモリ `no-silent-fallback`。
