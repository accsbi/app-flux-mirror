...
for (const lang of LANGS) {
  test.describe(`menu [${lang}]`, () => {
    ...
    test(`APP-002 VER 表示が非空（memory は config 静的値 1.0.0・Flutter 未移行）[${lang}]`, async ({ page }, info) => {
      // memory はまだ Flutter 未移行のため版数は config 静的値(1.0.0)。__APP_VERSION__ 同期は対象外。
      const ver = page.locator('standalone-game-menu .version-text')
      await expect(ver).toBeVisible()
      await expect(ver).toContainText('1.0.0')
      await snapAttach(page, info, `APP-002-${lang}`)
    })

    test(`MENU-001 START → 盤面のステージ選択(練習+STAGE1..10=11)へ遷移 [${lang}]`, async ({ page }, info) => {
      ...
      await expect(page.locator('.stage-select-btn--practice')).toBeVisible()
    ...
    test(`MENU-004 Remove Ads → 広告削除モーダル表示・タイトル非空 [${lang}]`, async ({ page }, info) => {
    ...
    test(`MENU-004b Remove Ads → Cancel で閉じられる [${lang}]`, async ({ page }, info) => {
      await page.locator('.menu-btn').nth(3).click()
      const panel = page.locator('remove-ads-dialog-panel')
      await expect(panel).toBeVisible()
    ...
    // MENU-005 / MENU-006 = FIX(2026-06-28): 旧 NG（ハンドラ未配線でタップ無反応）を修正済み。
    // memorymonsters-standalone-app に @menu-other-games / @menu-news を high-low と同型で配線し、
    // 共通部品 other-games-modal-panel / news-info-modal-panel を開く。ここでは「タップでモーダルが開き、
    // 閉じてメニューへ戻る」を期待値として固定する（旧 NG＝遷移欠落を OK＝開閉可能へ更新）。
    test(`MENU-005 Other Card Games → モーダル表示→閉じてメニュー復帰（FIX）[${lang}]`, async ({ page }, info) => {
      const btn = page.locator('.menu-btn').nth(4)
      ...
      await panel.locator('.top-close-btn').click()
      await expect(panel).toBeHidden()
      await expect(page.locator('standalone-game-menu')).toBeVisible()
    })

    test(`MENU-006 News/Updates → モーダル表示→閉じてメニュー復帰（FIX）[${lang}]`, async ({ page }, info) => {
      const btn = page.locator('.menu-btn').nth(5)
      await expect(btn).toBeVisible()
      expect((await btn.innerText()).trim().length).toBeGreaterThan(0)
      await btn.click()
