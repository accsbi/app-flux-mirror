...
// LANG: 言語切替（settings の language-select）。切替後に localStorage 更新＋メニュー文言が再描画される。
test.describe('language switch', () => {
  ...
  for (const { from, to } of cases) {
    test(`LANG-001 ${from}→${to} 言語切替で保存値とメニュー文言が変わる`, async ({ page }, info) => {
      await gotoMenu(page, from)
      const guideBefore = (await page.locator('.menu-btn').nth(1).innerText()).trim()
      await page.locator('.menu-btn').nth(2).click()
      const panel = page.locator('settings-panel')
      ...
      const guideAfter = (await page.locator('.menu-btn').nth(1).innerText()).trim()
      // 言語が変われば Guide ボタンの文言も変わる（同言語ケースは無いので必ず差分）。
      expect(guideAfter).not.toBe(guideBefore)
