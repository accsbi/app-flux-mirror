# Classic Simple Poker (Five-card draw) {Google / APP}
...
* ファイブカードドローポーカーの一般的なルールを採用しています。
* 残したいカードを[HOLD]、交換したいカードを[DRAW]を表示させて、SUBMITしてください。
* 強い役ほど、掛けたCOINに対して払い戻しが増えます。
* ダブルアップやオプションルール等はございません。
* 詳しいルールはガイドから確認できます。
...
A simple five-card draw poker game with a classic look.
...
* Choose your bet points, then you are dealt 5 cards.
* Tap the cards you want to swap, keep the rest (HOLD).
* Tap DRAW to swap, then SUBMIT to score your hand.
* The better your hand, the more points you win.
...
* 选择下注点数后，会发给你 5 张牌。
* 点选想换掉的牌，想留下的牌保持（HOLD）。
* 点 DRAW 交换，再点 SUBMIT 判定牌型。
* 牌型越强，赢得的点数越多。
...
**Classic Simple Poker** 是一款可以獨自輕鬆遊玩、容易上手的撲克遊戲。

選擇要下注的點數，換掉不需要的牌，用做成的牌型賺取點數。

無需註冊，也沒有線上對戰。牌型分數表始終顯示在畫面上，即使第一次玩也能輕鬆上手。所有牌皆為隨機發放。
...
* 選擇下注點數後，會發給你 5 張牌。
* 點選想換掉的牌，想留下的牌保持（HOLD）。
* 點 DRAW 交換，再點 SUBMIT 判定牌型。
* 牌型越強，贏得的點數越多。
...
一款外觀經典、玩法簡單嘅五張抽換撲克遊戲。

### [ 應用程式說明 ] {all}

**Classic Simple Poker** 係一款可以自己一個人輕鬆玩、好易上手嘅撲克遊戲。

揀好想下注嘅點數，換走唔要嘅牌，用砌到嘅牌型嚟賺點數。

唔使註冊，亦冇網上對戰。牌型分數表成日都喺畫面上面顯示，就算第一次玩都好易上手。啲牌全部都係隨機派。
...
* 揀好下注點數之後，就會派 5 張牌畀你。
* 撳一下想換走嘅牌，想留低嘅牌就保持（HOLD）。
* 撳 DRAW 換牌，再撳 SUBMIT 判定牌型。
* 牌型越強，贏到嘅點數就越多。
